...
\begin{frame}{Problema de ordenamiento}
    \begin{itemize}
        ...
        \begin{itemize}
            \item<2-> \blue{reflexiva}: \tabto{15ex} \uncover<3->{$\forall e_i\in Y$, $e_i\leq e_i$}
            \item<2-> \blue{anti-simétrica}: \tabto{15ex} \uncover<4->{$\forall e_i,e_j\in Y$, $e_i\leq e_j\wedge e_j\leq e_i\Rightarrow e_i=e_j$}
            \item<2-> \blue{transitiva}:\tabto{15ex} \uncover<5->{$\forall e_i,e_j,e_k\in Y$, $e_i\leq e_j\wedge e_j\leq e_k\Rightarrow e_i\leq e_k$}
        \end{itemize}
